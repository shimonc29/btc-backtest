...
from strategy_v4 import DEFAULTS, add_features, target_weights
...
def run_backtest(params: dict | None = None) -> dict:
    ...
    for i, date in enumerate(dates):
        if pending_weights is not None:
            ...
            regime_counts[pending_regime or "UNKNOWN"] = regime_counts.get(pending_regime or "UNKNOWN", 0) + 1
        ...
        if is_month_end and next_date is not None:
            pending_weights, pending_regime = target_weights(features, spy_features, date, p)
    ...
    summary = {
        "strategy": "V4 regime ensemble core + relative-strength satellites",
...
def main() -> None:
    ...
    outdir.joinpath("v4_summary.json").write_text(json.dumps(result["summary"], indent=2), encoding="utf-8")
    result["equity"].to_csv(outdir / "v4_equity.csv", index=False)
    pd.DataFrame(result["rebalances"]).to_csv(outdir / "v4_rebalances.csv", index=False)
