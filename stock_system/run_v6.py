...
def run_backtest(params: dict | None = None, raw_data: dict[str, pd.DataFrame] | None = None,
    ...
    spy_window = spy.loc[dates[0]:dates[-1], "Close"]
...
def main() -> None:
    ...
    outdir.joinpath("v6_summary.json").write_text(json.dumps(result["summary"], indent=2), encoding="utf-8")
    result["equity"].to_csv(outdir / "v6_equity.csv", index=False)
    pd.DataFrame(result["rebalances"]).to_csv(outdir / "v6_rebalances.csv", index=False)
