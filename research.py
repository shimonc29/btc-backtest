...
def period_slice(df, start=None, end=None):
    ...
    if "open time" in cols:
        ...
        raise ValueError("CSV needs 'Open time', 'timestamp', or 'datetime' for period research")
...
def run_period(name, df, equity):
    trades, stats = backtest(df, initial_equity=equity)
    return {
        "period": name,
        "bars": len(df),
        "trades": stats["trades"],
...
def main():
    ...
    periods = [
        ...
        ("2023-2024 IN-SAMPLE", "2023-01-01", "2025-01-01"),
    ...
    out.to_csv("research_summary.csv", index=False)
